...
def test_viewer_sees_customer_and_hr_only(seeded_backend, viewer):
    nav = service(seeded_backend, viewer).navigation()
    assert [i.function.name for i in nav] == [CUSTOMER, HR]  # sorted by name
    assert _nav(nav) == {
        HR: (Role.VIEWER, ["employment_types"]),
        CUSTOMER: (Role.VIEWER, ["service_areas", "survey_questions"]),
    }
    assert all(isinstance(i.function, FunctionDef) for i in nav)
    assert nav[0].function.display_name == "Customer Survey & Service Improvement"
    assert nav[0].function.form_count == 2
    assert nav[0].function.domain == "customer" and nav[1].function.domain == "people"


def test_editor_sees_customer_as_editor_and_finance_as_viewer(seeded_backend, editor):
    nav = service(seeded_backend, editor).navigation()
    assert _nav(nav) == {
        FINANCE: (Role.VIEWER, ["cost_centres", "gl_account_mappings"]),
        CUSTOMER: (Role.EDITOR, ["service_areas", "survey_questions"]),
    }
...
def test_admin_sees_everything(seeded_backend, admin):
    ...
    assert _nav(nav) == {
        ...
        CUSTOMER: (Role.ADMIN, ["service_areas", "survey_questions"]),
    }
    assert svc.permissions.can_create_function
    assert [d.function.name for d in svc.visible_functions()] == [CUSTOMER, FINANCE, HR]
...
def test_grouped_navigation_follows_domain_order_and_puts_unassigned_last(seeded_backend, admin):
    ...
    assert [g.domain.name for g in groups] == ["customer", "finance", "people"]  # research has no functions
    assert [[f.function.name for f in g.functions] for g in groups] == [[CUSTOMER], [FINANCE], [HR]]
    assert groups[0].domain.title == "Customer" and not groups[0].is_unassigned
...
def test_search_matching_a_form_filters_forms(seeded_backend, admin):
    svc = service(seeded_backend, admin)
    assert _nav(svc.navigation("survey_questions")) == {CUSTOMER: (Role.ADMIN, ["survey_questions"])}
    # form description
...
def test_search_can_hit_several_functions(seeded_backend, admin):
    ...
    # "reference" appears in the customer and finance descriptions and in the hr function name
    assert set(_nav(nav)) == {CUSTOMER, FINANCE, HR}
    ...
    assert _nav(nav)[CUSTOMER][1] == ["service_areas", "survey_questions"]
...
def test_blank_search_returns_everything(seeded_backend, editor, search):
    ...
    assert set(_nav(nav)) == {CUSTOMER, FINANCE}
...
def test_search_results_do_not_mutate_the_full_listing(seeded_backend, admin):
    ...
    full = svc.navigation()
    assert _nav(full)[CUSTOMER][1] == ["service_areas", "survey_questions"]


def test_get_form_respects_visibility(seeded_backend, viewer):
    svc = service(seeded_backend, viewer)
    form = svc.get_form(CUSTOMER, "survey_questions")
    ...
    with pytest.raises(NotFoundError):
        svc.get_form(CUSTOMER, "no_such_form")


def test_get_form_on_unknown_function_is_permission_denied(seeded_backend, admin):
    svc = CatalogService(seeded_backend, admin, Permissions({CUSTOMER: Role.VIEWER}))
...
def test_navigation_reflects_new_grants_when_permissions_are_refreshed(seeded_backend, viewer):
    ...
    stale = CatalogService(seeded_backend, viewer, Permissions({CUSTOMER: Role.VIEWER}))
    assert set(_nav(stale.navigation())) == {CUSTOMER}
