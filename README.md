...
reference data. The hierarchy is **domain > function > form**: a domain is a business
classifier aligned with the organisation's data domains, a function is a Unity Catalog
schema, and every "form" is a governed Delta table in that schema. Access is the schema's
Unity Catalog grants. The app is metadata-driven: it discovers domains, functions and forms
at runtime and renders editable grids from the table definitions.
...
* **Registry**: `_catalog.domains`, `_catalog.functions` and `_catalog.forms` record every
  domain, function and form with display name, description, owner and documentation link.
...
make seed               # creates data/rdm.duckdb with four demo domains and three functions
...
src/rdm/
  ...
  services/               FormService, CatalogService, Draft (row-id edit tracking, bulk update,
                          restore), Excel import
  ui/                     Dash app: shell, routing, pages (home, function, domains, form, wizard,
                          help), AG Grid configuration
